...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentServiceImpl(PaymentProducerService paymentProducerService, PaymentRepository paymentRepository, AccountRepository accountRepository) {
    ...
    public PaymentResponseDto initiatePayment(PaymentRequestDto request) {
        ...
        Account source = accountRepository.findById(sourceId).orElseThrow(() -> new AccountException("Source account not found"));
        Account target = accountRepository.findById(targetId).orElseThrow(() -> new AccountException("Target account not found"));

        if (source.getUser().getId().equals(target.getUser().getId())) {
            throw new PaymentException("Accounts belong to the same user. Use the transfer endpoint instead.");
        }
