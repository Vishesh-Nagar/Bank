...
const fetchAccounts = async () => {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewAccount({ accountHolderName: '', balance: 0, accountType: 'SAVINGS' });
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data || 'Failed to withdraw');
    }
  };
    ...
    <div className="dashboard">
      ...
      {transactionModal && selectedAccount && (
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h2>{transactionModal === 'deposit' ? '💰 Deposit Money' : '💸 Withdraw Money'}</h2>
            
            <div className="transaction-info">
              <p><strong>Account:</strong> {selectedAccount.accountHolderName}</p>
              ...
              <div className="transaction-preview">
                <p>New Balance: ${
                  transactionModal === 'deposit'
                    ? (selectedAccount.balance + parseFloat(transactionAmount)).toFixed(2)
                    : (selectedAccount.balance - parseFloat(transactionAmount)).toFixed(2)
                }</p>
              </div>
            )}
            ...
            <div className="modal-actions">
              <button
                onClick={transactionModal === 'deposit' ? handleDeposit : handleWithdraw}
                className={`btn ${transactionModal === 'deposit' ? 'btn-success' : 'btn-warning'}`}
                disabled={!transactionAmount || parseFloat(transactionAmount) <= 0}
              >
                {transactionModal === 'deposit' ? 'Deposit' : 'Withdraw'}
              </button>
              <button 
                onClick={() => {
                  setTransactionModal(null);
