...
// Create account
export const createAccount = async (account: AccountCreateDto): Promise<AccountDto> => {
...
// Deposit money
export const deposit = async (id: number, amount: number): Promise<AccountDto> => {
  const response = await api.put(`/accounts/${id}/deposit`, { amount });
  return response.data;
};

// Withdraw money
export const withdraw = async (id: number, amount: number): Promise<AccountDto> => {
